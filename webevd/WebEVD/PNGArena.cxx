...
namespace evd {
    ...
    for (int newdim = kArenaSize / 2; newdim >= 1; newdim /= 2) {
      ...
      for (int y = 0; y < newdim; ++y) {
        for (int x = 0; x < newdim; ++x) {
          ...
          for (int c = 0; c < 3; ++c)
            dest[i * 4 + c] = tota > 0 ? std::min(totc[c] / tota, 255) : 0;
  ...
  void AnalyzeArena(const PNGArena& bytes)
  {
    for (int blockSize = 1; blockSize <= PNGArena::kArenaSize; blockSize *= 2) {
      ...
      std::cout << "With block size = " << blockSize << " "
                << double(nfilled) / ((PNGArena::kArenaSize * PNGArena::kArenaSize) /
                                      (blockSize * blockSize) * bytes.data.size())
                << " of the blocks are filled" << std::endl;
    }
  }
